import rules from "./quick-rules.json";
import {
...
function renderItems(items: any[], depth: number = 0): JSX.Element {
  if (!items) return <></>;
  return (
...
export default function FieldHandbook() {
  const mutedColor = "text-muted-foreground";
  return (
    <main className="p-12">
      <div className="flex flex-col w-4/5 mx-auto">
        <h1 className="text-5xl font-extrabold mb-2 tracking-tight">Field Handbook</h1>
        ...
        {(rules.sections as {
          icon: string;
          title: string;
          items: ({ text: string; items: { text: string; }[]; } | { text: string; items?: undefined; })[];
          html?: string;
        }[]).map(({ icon, title, items, html }) => {
